    ...
    internal class DatabaseMethods
    {
        ...
        public List<LicensePlateDataDocument> GetLicensePlatesToExport()
        {
            ...
            using (_client = new DocumentClient(new Uri(_endpointUrl), _authorizationKey))
            {
                ...
                licensePlates = _client.CreateDocumentQuery<LicensePlateDataDocument>(collectionLink, new FeedOptions() 
                { EnableCrossPartitionQuery = true, MaxItemCount = 100 }).Where(l => l.exported == false).ToList();
